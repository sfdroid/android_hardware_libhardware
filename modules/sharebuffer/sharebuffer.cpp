...
int connect_to_renderer()
{
    ...
    if(fd < 0)
    {
        LOGE("error creating socket stream");
        return -1;
    }
    ...
    {
        LOGE("error connecting to renderer: %s", strerror(errno));
        close(fd);
        return -1;
    }
...
int recv_status(int fd, int *failed)
{
    ...
    {
        LOGE("message_buffer is not a 0 terminated string");
        return -1;
    }
    ...
    else
    {
        ...
        {
            LOGE("unknown status: %s", message_buffer);
        }
        *failed = 1;
        return 0;
    }
...
static int sb_post(struct sharebuffer_device_t* dev, buffer_handle_t buffer, uint32_t width, uint32_t height, uint32_t stride, int32_t pixel_format)
{
    ...
    if(m->fd_renderer >= 0)
    {
        ...
        if(!already_sent)
        {
            ...
            {
                LOGW("failed to send buffer notification: %s", strerror(errno));
                goto exit_error;
            }

            if(send_native_handle(m->fd_renderer, buffer, width, height, stride, pixel_format) < 0)
            {
                LOGW("sending buffer failed: %s", strerror(errno));
                goto exit_error;
            }

            if(recv_status(m->fd_renderer, &failed) < 0)
            {
                LOGW("recv_status failed: %s", strerror(errno));
                goto exit_error;
            }
        ...
        else
        {
            ...
            if(send(m->fd_renderer, buf, 1, 0) < 0)
            {
                LOGW("failed to send old buffer notification: %s", strerror(errno));
                goto exit_error;
            }
...
int sharebuffer_register_buffer(gralloc_module_t const* module,
        buffer_handle_t handle)
{
    LOGW("sharebuffer_register_buffer stub");
    return 0;
}
...
int sharebuffer_unregister_buffer(gralloc_module_t const* module,
        buffer_handle_t handle)
{
    LOGW("sharebuffer_unregister_buffer stub");
    return 0;
}
        ...
        hw_device_t** device)
{
    ...
    if (!strcmp(name, GRALLOC_HARDWARE_GPU0)) {
        LOGE("FATAL: Tried to load sharebuffer module with %s as argument.", name);
    } else {
